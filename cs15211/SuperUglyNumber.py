__source__ = 'https://leetcode.com/problems/super-ugly-number/'
...
class Solution {
    public int nthSuperUglyNumber(int n, int[] primes) {
        ...
        for (int j = 0; j < primes.length; j++) {
            pq.add(new Ugly(primes[j], 1, primes[j]));
        }
        ...
        for (int i = 1; i < n ;i++) {
            ...
            while (pq.peek().val == ugly[i]) {
                Ugly cur = pq.poll();
                pq.add(new Ugly(ugly[cur.idx] * cur.p, cur.idx + 1, cur.p));
            }
        }
        return ugly[n -1];
    }
    ...
    class Ugly implements Comparable<Ugly> {
        int val, idx ,p;
        public Ugly (int val, int idx, int p) {
            this.val = val;
            this.idx = idx;
            this.p = p;
        }
        ...
        @Override
        public int compareTo(Ugly that) {
