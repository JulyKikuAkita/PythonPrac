__source__ = 'https://leetcode.com/problems/minimum-number-of-k-consecutive-bit-flips/'
# Time:  O(N)
# Space: O(1)
#
# Description: Leetcode # 995. Minimum Number of K Consecutive Bit Flips
#
# In an array A containing only 0s and 1s,
# a K-bit flip consists of choosing a (contiguous) subarray of length K
# and simultaneously changing every 0 in the subarray to 1, and every 1 in the subarray to 0.
#
# Return the minimum number of K-bit flips required so that there is no 0 in the array.
# If it is not possible, return -1.
#
# Example 1:
#
# Input: A = [0,1,0], K = 1
# Output: 2
# Explanation: Flip A[0], then flip A[2].
# Example 2:
#
# Input: A = [1,1,0], K = 2
# Output: -1
# Explanation: No matter how we flip subarrays of size 2, we can't make the array become [1,1,1].
# Example 3:
#
# Input: A = [0,0,0,1,0,1,1,0], K = 3
# Output: 3
# Explanation:
# Flip A[0],A[1],A[2]: A becomes [1,1,1,1,0,1,1,0]
# Flip A[4],A[5],A[6]: A becomes [1,1,1,1,1,0,0,0]
# Flip A[5],A[6],A[7]: A becomes [1,1,1,1,1,1,1,1]
#
#
# Note:
#
# 1 <= A.length <= 30000
# 1 <= K <= A.length
#
import unittest
# 108ms 100%
class Solution(object):
    def minKBitFlips(self, A, K):
        """
        :type A: List[int]
        :type K: int
        :rtype: int
        """
        cur = res = 0
        for i in range(len(A)):
            if i >= K and A[i - K] == 2:
                cur -= 1
            if (cur % 2) == A[i]:
                if i + K > len(A):
                    return -1
                A[i] = 2
                cur, res = cur + 1, res + 1
        return res

class TestMethods(unittest.TestCase):
    def test_Local(self):
        self.assertEqual(1, 1)

if __name__ == '__main__':
    unittest.main()

Java = '''
# Thought: https://leetcode.com/problems/minimum-number-of-k-consecutive-bit-flips/solution/
# Approach 1: Greedy + Events
# Complexity Analysis
# Time Complexity: O(N), where N is length of A.
# Space Complexity: O(N) 
# 9ms 100%
class Solution {
    public int minKBitFlips(int[] A, int K) {
        int N = A.length;
        int[] hint = new int[N];
        int ans = 0, flip = 0;

        // When we flip a subarray like A[i], A[i+1], ..., A[i+K-1]
        // we can instead flip our current writing state, and put a hint at
        // position i+K to flip back our writing state.
        for (int i = 0; i < N; ++i) {
            flip ^= hint[i];
            if (A[i] == flip) {  // If we must flip the subarray starting here...
                ans++;  // We're flipping the subarray from A[i] to A[i+K-1]
                if (i + K > N) return -1;  //If we can't flip the entire subarray, its impossible
                flip ^= 1;
                if (i + K < N) hint[i + K] ^= 1;
            }
        }
        return ans;
    }
}

# https://leetcode.com/problems/minimum-number-of-k-consecutive-bit-flips/discuss/238609/JavaC%2B%2BPython-One-Pass-and-O(1)-Space
# Explanation:
# One pass, count the cur filp numbers in a sliding window of size K.
# If cur is even and A[i] is 0, we need to flip.
# If cur is odd and A[i] is 1, we need to flip.
# O(N) time and O(1) space
#10ms 100%
class Solution {
    public int minKBitFlips(int[] A, int K) {
        int cur = 0, res = 0;
        for (int i = 0; i < A.length; i++) {
            if (i >= K) cur -= A[i - K] / 2;
            if ((cur & 1 ^ A[i]) == 0) {
                if (i + K > A.length) return -1;
                A[i] += 2;
                cur++;
                res++;
            }
        }
        return res;
    }
}
'''
