...
# as to have as little impact on subsequent intervals as possible.
...
public class Solution {
    public int eraseOverlapIntervals(Interval[] intervals) {
            ...
            for (int i = 1; i < intervals.length; i++) {
                if (end <= intervals[i].start) { //need to include equals
                    count++;
                    end = intervals[i].end;
                }
            }
...
public class Solution {
    public int eraseOverlapIntervals(Interval[] intervals) {
            ...
            for (int i = 1; i < intervals.length; i++) {
                if (end > intervals[i].start) { //need to include equals
                    count++;
                    end = Math.min(end, intervals[i].end); //erase the one with larger end time
                } else {
                    end = intervals[i].end;
                }
            }
